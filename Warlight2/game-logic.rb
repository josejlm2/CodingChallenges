...
class GameLogic
  ...
  def place_armies(time)
	...
	#puts @world_map.my_regions.min
	#puts @world_map.my_regions.order("forces")
	smallest_forces = self.@world_map.min_by {|my_regions| my_regions.forces}
	puts smallest_forces.id
  ...
  def attack_or_transfer(time)
	...
	#puts attack_from.each{|region| puts "Region ID= #{region.id}, neighbours: #{region.neighbours.sample.id}, forces:#{region.forces - 1}"}
  end
  ...
  def pick_starting_region(time, region_ids)
    region_id = region_ids.sample #default is random
	
	region_id = region_ids.min
	
    #region_ids.each do |id| 
	#	@world_map.super_regions.each do |super_regions_id|
		   
		
	#		if (super_regions_id == id )
	#			region_id = id #look for a super region and start there
	#		end
	#	end
	#end
	
	#puts @world_map.super_regions.inspect
